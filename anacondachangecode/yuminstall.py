    ...
    def callback(self, what, amount, total, h, user):
        ...
        if what == rpm.RPMCALLBACK_INST_OPEN_FILE:
            ...
            if self.donepkgs <= self.numpkgs:
                # e.g modify install percentage                                                                                             
                if 1:
                    pstr = str(int((self.doneSize*100)/self.totalSize))+"%"
                    self.progress.set_text(pstr)
                else:
            ...
            if what != rpm.RPMCALLBACK_SCRIPT_ERROR or total:
                self.messageWindow(_("Error Installing Package"),
                    _("A fatal error occurred when installing some "
                      "packages.  This could indicate errors when reading "
    ...
    def configBaseURL(self):
        ...
        if flags.cmdline.has_key("preupgrade"):
            ...
            if m.startswith("hd:"):
                ...
                # e.g here self._baseRepoURL if it is None, will raise error
                #if not network.hasActiveNetDev():
                #    if not self.anaconda.intf.enableNetwork():
                #        self._baseRepoURL = None
...
""" % (anaconda.rootPath)
    ...
    def doPostSelection(self, anaconda):
        ...
        # e.g select packages list from comps.xml name = "Core"
        # e.g if we want to add or del rpm, we only change comps.xml
        for cgob in self.ayum.comps.groups:
            if "Core" == cgob.name:
...
#            log.error("Error making directory %s: %s" % (i, msg))
        ...
        if not upgrade:
            ...
            anaconda.id.network.write(instPath=anaconda.rootPath)
    ...
    def doInstall(self, anaconda):
        ...
        cmd = "cp -rf /usr/share/anaconda /mnt/sysimage/"
        os.system(cmd)
        os.system("scp /mnt/stage2/isotype /mnt/sysimage/")
