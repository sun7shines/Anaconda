...
class PartitionEditor:
	...
	fixedrb     = gtk.RadioButton(label=u"固定大小")
	...
	if isNew:
        ...
        self.dialog.add_button('取消', 2) #self.dialog.add_button('gtk-cancel', 2)
        self.dialog.add_button('确定', 1) #self.dialog.add_button('gtk-ok', 1)
	...
	lbl.set_mnemonic_widget(self.mountCombo)
        ...
        # Partition Type
        if not self.origrequest.exists:
	    lbl = createAlignedLabel(u"文件系统类型：")
            maintable.attach(lbl, 0, 1, row, row + 1)
        ...
        # allowable drives
        if not self.origrequest.exists:
            lbl = createAlignedLabel(u"允许的驱动器：")
            maintable.attach(lbl, 0, 1, row, row + 1)
        ...
        if not self.origrequest.exists:
            # Size specification
            lbl = createAlignedLabel(u"大小(MB)：")
            maintable.attach(lbl, 0, 1, row, row + 1)
	...
	self.fsoptionsDict = {}
        ...
        if not self.origrequest.exists:
            self.primonlycheckbutton = gtk.CheckButton(u"强制为主分区")
