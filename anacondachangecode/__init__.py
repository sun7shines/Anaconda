        ...
        if not flags.livecdInstall and not storage.protectedDevices and not anaconda.protected:
            if anaconda.id.getUpgrade():
                ...
                anaconda.intf.messageWindow(_("Unknown Device"),
                    ...
                    type="custom", custom_buttons = [_(u"_退出安装")])
                sys.exit(1)
    else:
        storage.reset()

    if not storage.disks:
        rc = anaconda.intf.messageWindow(_(u"没有发现磁盘"),
                _(u"没有发现可用的磁盘。"),
                type="custom",
                custom_buttons = [_(u"后退"), _(u"_退出安装")],
                default=0)
        if rc == 0:
...
def storageComplete(anaconda):
    ...
    if not anaconda.id.storage.encryptionPassphrase:
        ...
        anaconda.id.storage.retrofitPassphrase = True
        anaconda.id.storage.encryptedAutoPart = True
        anaconda.id.storage.encryptionRetrofit = True
    ...
    if iutil.isS390() and \
       ...
       not anaconda.id.storage.mountpoints['/'].singlePV:
        ...
        if rc == 0:
            return DISPATCH_BACK
        sys.exit(1)
    if anaconda.id.installationtype==True:
        rc=1
    else:
