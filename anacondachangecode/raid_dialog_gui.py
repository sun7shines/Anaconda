...
class RaidEditor:
	...
	# raid minors
	lbl = createAlignedLabel(u"RAID 设备：")	
	maintable.attach(lbl, 0, 1, row, row + 1)
	...
	# raid level
	lbl = createAlignedLabel(u"RAID 级别：")
	maintable.attach(lbl, 0, 1, row, row + 1)
	...
	# raid members
	lbl=createAlignedLabel(u"RAID 成员：")
	maintable.attach(lbl, 0, 1, row, row + 1)
	...
	if not format.exists and not origrequest.exists:
	    self.formatButton = gtk.CheckButton(u"格式化分区吗？")
	    if not format.type:
