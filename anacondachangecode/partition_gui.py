...
class Stripe:
    ...
    def putOnCanvas(self, yoff):
        ...
        self.canvas_text = self.canvas.root().add(gnomecanvas.CanvasText,
                x=0.0, y=yoff, font="微软雅黑", size_points=9)
...
class MessageGraph:
    ...
    def display(self):
        ...
        self.canvas_text = self.canvas.root().add(gnomecanvas.CanvasText,
                x=0.0, y=20, font="微软雅黑", size_points=16)
...
class PartitionWindow(InstallWindow):
	...
	al.add(info1)
        ...
        sl = gtk.Alignment(0.0,0.0)
        sl.add(sw)
        vbox.pack_start(al, False)
	
        vbox.pack_start(sl, True, True, 9)
...
#                          "settings.")
    ...
    def getScreen(self, anaconda):
        ...
        ops = ((u"创建", self._createCB, "create"),
               (u"编辑", self._editCB, "edit"),
               (u"删除", self._deleteCB, "delete"),
               (u"重设", self._resetCB, "reset"))
        ...
        path = "/usr/share/anaconda/pixmaps/icon10_harddisk_partition_custom.png"
        i.set_from_file(path)
        i.show()
        fixed.put(i,45,15)
        label1 = gtk.Label()
        label1.set_markup("<span font_desc=' 16' weight='bold' foreground='"+fontcolor+"'><b>自定义分区:</b></span>")
        fixed.put(label1,160,23)
        label2 = gtk.Label()
        label2.set_markup("<span font_desc=' 11.5' weight='bold' foreground='"+fontcolor+"'><b>手动配置硬盘分区</b></span>")
        ...
        ##################################################################
        label = gtk.Label()
        label.set_markup("<span font=' 10.5' foreground='#333333' size='large'><b>请选择驱动器</b></span>")
        tmphbox = gtk.HBox()
        tmphbox.pack_start(label,False,False,50)
        vbox.pack_start(tmphbox,False,False,20)
        """vbox.pack_start(swt,False,False,0)"""
        ##############################################################
        swb.set_size_request(950,-1)
        tmphbox = gtk.HBox()
        tmphbox.pack_start(swb,False,False,50)
        vbox.pack_start(tmphbox,True,True,0)
        #############################################################
        tmphbox = gtk.HBox()
        tmphbox.pack_end(buttonBox,False,False,90)
        vbox.pack_end(tmphbox, False, False,30)
